...
def build_project_materialization_payload(session: NovelCreationSession) -> dict[str, Any]:
    ...
    characters = _dict(stages.get("characters", {}).get("data")) or derive_stage(
        session, "characters"
    )
    ...
    if opening_state.get("status") == "confirmed":
        validate_opening_outline(
            opening, volume_index=volume_index, character_index=creation_character_index(session)
        )
    ...
    volumes = [
        ...
        if entity.artifact_key == "macro_outline"
        and entity.entity_type == "volume"
    ...
    protagonist = next(
        (row for row in character_rows if row.get("role_type") == "protagonist"),
    ...
    all_world.extend(
        item
        for item in _list(locations.get("entries"))
        if isinstance(item, dict) and _text(item.get("title")) not in known_titles
    )
    project_payload.update(
        {
            ...
            "writing_style": _text(
                world.get("writing_style") or project_payload.get("writing_style")
            ),
            "world_tone": _text(world.get("world_tone") or project_payload.get("world_tone")),
            "story_structure": _text(
                world.get("story_structure") or project_payload.get("story_structure")
            ),
            "pacing": _text(world.get("pacing") or project_payload.get("pacing")),
            "style_rules": _rule_lines(
                world.get("style_rules", project_payload.get("style_rules"))
            ),
            "forbidden_patterns": _rule_lines(
                world.get("forbidden_patterns", project_payload.get("forbidden_patterns"))
            ),
