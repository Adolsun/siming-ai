...
- 作品库与删除操作共用真实同步状态：尚未上传的项目包显示“仅本机”，无需 Gateway 即可删除，同时清理副本、保留的原包和待同步记录；配置了 Gateway 也不会为了删除先上传。已同步作品仍通过 PC 同一删除 API 处理；上传中断、回执未确认时需联网核验。删除、项目包导入与同步串行执行，防止后台上传在删除后重新创建作品。
