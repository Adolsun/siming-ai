...
describe('CatalogingPage job ownership', () => {
  ...
  it('upserts durable candidates on reconnect and ignores events from a replaced connection', async () => {
    const active = job('job-a', 'model-a')
    const activeRun = run('run-a', 'job-a', '当前章节')
    const candidate = { id: 'candidate-1', job_id: active.id, chapter_run_id: activeRun.id,
      chapter_id: activeRun.chapter_id, item_type: 'chapter_summary', payload: { summary_text: '已有计划' }, status: 'pending' }
    api.stream.mockImplementation(() => vi.fn())
    api.get.mockImplementation((url: string) => {
      ...
      if (url.endsWith('/facts')) return Promise.resolve({ data: { data: { items: [], total: 0 } } })
      if (url.endsWith('/candidates')) return Promise.resolve({ data: { data: { items: [candidate], total: 1 } } })
      return Promise.reject(new Error(`unexpected GET ${url}`))
    })
    const view = render(<CatalogingPage projectId="project-1" />)
    fireEvent.click(await screen.findByRole('button', { name: '载入任务' }))
    expect(await screen.findByText('已暂存 1 条候选')).toBeInTheDocument()
    await waitFor(() => expect(api.stream).toHaveBeenCalledOnce())
    const firstCallback = api.stream.mock.calls[0][2]
    const firstError = api.stream.mock.calls[0][3]
    await act(async () => {
      firstCallback(JSON.stringify({ type: 'candidate_created', candidate, run: activeRun, job: active }))
      firstCallback(JSON.stringify({ type: 'candidate_created', candidate, run: activeRun, job: active }))
    })
    expect(screen.getByText('已暂存 1 条候选')).toBeInTheDocument()
    ...
    await act(async () => {
      ...
      api.stream.mock.calls[1][2](JSON.stringify({ type: 'candidate_created', candidate, run: activeRun, job: active }))
    })
